...
const bundleBookings = (bookingsList) => {
    const groups = {};
    bookingsList.forEach(item => {
        const key = `${item.phone}_${item.booking_date}_${item.time_slot}_${item.duration_hours}_${item.service_name}_${item.status}`;
        if (!groups[key]) {
            groups[key] = {
                ...item,
                ids: [item.id],
...
export default function AdminDashboard() {
    ...
    const load = async () => {
        try {
            ...
            setStats(s.data);
            setBookings(bundleBookings(b.data.items || []));
    ...
    const getWhatsAppConfirmationUrl = (booking) => {
        ...
        const msg = `Booking Confirmed!*\nHi ${booking.name}, your booking ${serviceName} on ${booking.booking_date} at ${booking.time_slot} is confirmed. See you at Gamebrew!`;
    ...
    const updateStatus = async (booking, status) => {
        ...
        try {
            ...
            const idsToUpdate = booking.ids || [booking.id];
            await Promise.all(idsToUpdate.map(id => api.patch(`/admin/bookings/${id}`, { status })));

            toast.success(`Booking(s) ${status}`);
            load();
                ...
                <nav className="flex-1 space-y-1">
                    {TABS.map((t) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-3 font-display uppercase text-xs tracking-widest transition-colors ${tab === t.id ? "bg-neon-red text-white" : "text-white/60 hover:text-white hover:bg-white/5"
                                }`}
                        >
                ...
                {tab === "bookings" && (
                    <div data-testid="admin-bookings-tab">
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-black/50">
                                    <tr className="text-left font-display uppercase tracking-widest text-xs text-white/50">
                                        <th className="p-4">Name</th><th className="p-4">Service</th><th className="p-4">Qty</th><th className="p-4">Date</th>
                                        <th className="p-4">Slot</th><th className="p-4">Amount</th><th className="p-4">Status</th><th className="p-4">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {bookings.map((b) => (
                                        <tr key={b.ids ? b.ids[0] : b.id} className="border-t border-white/5" data-testid={`booking-row-${b.ids ? b.ids[0] : b.id}`}>
                                            <td className="p-4">
                                                <div>{b.name}</div>
                                            ...
                                            <td className="p-4">
                                                <span className={`px-2 py-1 text-xs font-display uppercase tracking-wider ${b.status === "confirmed" ? "bg-green-500/20 text-green-400" :
                                                    b.status === "cancelled" ? "bg-red-500/20 text-red-400" :
                                                        "bg-yellow-500/20 text-yellow-400"
                                                    }`}>{b.status}</span>
                                            </td>
                                            <td className="p-4 flex gap-1">
                                                <button onClick={() => updateStatus(b, "confirmed")} data-testid={`confirm-${b.ids ? b.ids[0] : b.id}`} title="Confirm" className="p-1 text-green-400 hover:bg-green-500/20"><CheckCircle2 size={16} /></button>
                                                <button onClick={() => updateStatus(b, "cancelled")} data-testid={`cancel-${b.ids ? b.ids[0] : b.id}`} title="Cancel" className="p-1 text-red-400 hover:bg-red-500/20"><XCircle size={16} /></button>
                                            </td>
                                        </tr>
                                    ))}
                                    {!bookings.length && <tr><td colSpan={8} className="p-8 text-center text-white/40">No bookings yet</td></tr>}
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:hidden">
                            {bookings.map((b) => (
                                <div key={b.ids ? b.ids[0] : b.id} className="glass p-5" data-testid={`booking-card-${b.ids ? b.ids[0] : b.id}`}>
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <div className="font-display text-lg font-bold">{b.name}</div>
                                            <div className="text-white/50 text-xs">{b.phone}</div>
                                        </div>
                                        <span className={`px-2 py-1 text-[10px] font-display uppercase tracking-wider ${b.status === "confirmed" ? "bg-green-500/20 text-green-400" :
                                            b.status === "cancelled" ? "bg-red-500/20 text-red-400" :
                                                "bg-yellow-500/20 text-yellow-400"
                                            }`}>{b.status}</span>
                                    </div>
                                    <div className="space-y-1 text-sm text-white/70 mb-4">
                                        ...
                                        <div className="flex justify-between">
                                            <span>Quantity:</span> <span className="text-white font-bold">{b.quantity}</span>
                                        </div>
                                        <div className="flex justify-between">
